...
def do_sign(cred, pushbullet_token):
    # 获取绑定的角色列表
    ...
    for i in characters:
        ...
        if resp['code'] != 0:
            ...
            message = f'角色{i.get("nickName")}({i.get("channelName")})签到失败了！原因：{resp.get("message")}'
            # 检查返回的message是否包含"请勿重复签到"
            if "请勿重复签到" not in message:
                send_pushbullet_notification(pushbullet_token, "签到失败", message)
            continue
        
        awards = resp['data']['awards']
        for j in awards:
            res = j['resource']
            logging.info(f'角色{i.get("nickName")}({i.get("channelName")})签到成功，获得了{res["name"]}×{res.get("count") or 1}')
            message = f'角色{i.get("nickName")}({i.get("channelName")})签到成功，获得了{res["name"]}×{res.get("count") or 1}'
            send_pushbullet_notification(pushbullet_token, "签到成功", message)